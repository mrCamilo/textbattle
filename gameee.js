...
// the enemies get progressively harder in each area
function Enemy(name, strength, hitpoints, defense, luck) {
    this.name = name;
...
// the first area has 3 low level enemies
let enemy1 = new Enemy("Slime", 3, 12, 4, 5);
let enemy2 = new Enemy("Chocobo", 5, 8, 6, 3)
let enemy3 = new Enemy("Crawler", 7, 15, 4, 0);
...
function randomEnemy() {
        ...
        randomNumber = Math.floor(Math.random() * (+enemyArray.length - +0) + +0)
        enemy = enemyArray[randomNumber]; // pick a random enemy from this array
        enemyArray.splice(randomNumber, 1); // remove that enemy from the array using randomNumber variable
